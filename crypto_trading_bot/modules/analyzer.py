...
class MarketAnalyzer:
        ...
        try:
            ...
            if hasattr(self.config, 'trading_mode'):
                ...
                if self.config.trading_mode == TradingMode.FUTURES:
                    ...
                    min_candles_5m = 288  # Full 24h for spot
    ...
    def _calculate_adx(self, ohlcv: List[Dict], period: int) -> np.ndarray:
        ...
        di_plus = 100 * np.convolve(dm_plus, np.ones(period)/period, mode='valid') / (atr + np.float64(1e-10))
        di_minus = 100 * np.convolve(dm_minus, np.ones(period)/period, mode='valid') / (atr + np.float64(1e-10))
